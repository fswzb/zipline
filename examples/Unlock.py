...
unlock = pd.read_pickle(r"E:\Anaconda\Lib\site-packages\zipline_china\zipline\cache\EventData\Restricted Stock Unlock\restriction_unlock.pkl")              
#backtest setting"
...
def handle_data(context, data):
    ...
    if not hasattr(context, 'signal_time'):
        context.signal_time = {}
    for sym in data:
        if 'unlock_amount' in data[sym] and data[sym]['eventdate'] == context.datetime:    
            unlock = data[sym]['unlock_amount']*10000
            cap = data[sym]['cap']
            ratio = unlock/cap
    ...
    for sym in context.selected:
        ...
        record(rating_chg = data[sym]['unlock_amount'])
...
results = algo.run(datasource, overwrite_sim_params = False, optional_source = [unlock])
