...
class PerformancePeriod(object):
    ...
    def handle_delist(self, delist):
        #remove the delist position from our book
        if delist.sid in self.positions:
            # Make the position object handle the split. It returns the
            # leftover cash from a fractional share, if there is any.
            position = self.positions[delist.sid]
            self._position_amounts[delist.sid] = position.amount
            #self._position_last_sale_prices[delist.sid] = 0.0
    ...
    def calculate_performance(self):
        ...
        if total_at_start != 0:
            ...
            self.returns = 0.0
        #if abs(self.returns) > 0.11 and self.starting_cash != 100000.0:
            #print "period.py 260: pnl:%s, starting_value:%s, ending_value:%s"%(self.pnl, self.starting_value, self.ending_value)
            #print "positions: "
    ...
    def execute_transaction(self, txn):
        ...
        #print "txn_sid: %s"%(txn.sid)    
        #print "txn.price: %s, txn.amount: %s"%(txn.price, txn.amount)     
        #print "txn_datetime: %s"%(txn.dt)
        #print "period_cash_flow: %s\n\n"%(self.period_cash_flow)
