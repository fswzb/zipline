...
class PerformanceTracker(object):
    ...
    def to_dict(self, emission_type=None):
        ...
        print "cumulative_returns: %s\n"%(self.cumulative_performance.returns)
        return _dict
    ...
    def process_event(self, event):
        ...
        if event.type == zp.DATASOURCE_TYPE.TRADE:
            # update last sale
            self.sim_params.active_sids = self.sim_params.active_sids.union({event.sid})
            self.trade_count += 1
            if event.sid not in self.sim_params.sids:
                self.sim_params.sids.add(event.sid)
            for perf_period in self.perf_periods:
            ...
            new_dividend = pd.DataFrame(event.to_series()).T
            self.update_dividends(new_dividend)
        ...
        elif event.type == zp.DATASOURCE_TYPE.DELIST:
            if event.sid in self.sim_params.sids:
                self.sim_params.sids.remove(event.sid)
            for perf_period in self.perf_periods:
                perf_period.handle_delist(event)
    ...
    def handle_market_close_daily(self):
        ...
        self.market_open, self.market_close = \
            self.env.next_open_and_close(self.market_open)
