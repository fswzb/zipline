...
def transact_stub(slippage, commission, event, open_orders, cash):
    """
    This is intended to be wrapped in a partial, so that the
    slippage and commission models can be enclosed.
    """
    for order, transaction in slippage(event, open_orders, cash):
...
class SlippageModel(with_metaclass(abc.ABCMeta)):
    ...
    @property
    def available_cash_for_bar(self):
        return self._available_cash_for_bar
        
    @abc.abstractproperty
    def process_order(self, event, order):
        pass

    def simulate(self, event, current_orders, cash):

        self._volume_for_bar = 0
        self._available_cash_for_bar = cash
        ...
        for order in current_orders:
            ...
            if txn:
                #print "slippage177: txn amount:", txn.amount
                self._volume_for_bar += abs(txn.amount)
                self._available_cash_for_bar -= txn.price*txn.amount
                yield order, txn

    def __call__(self, event, current_orders, cash, **kwargs):
        return self.simulate(event, current_orders, cash, **kwargs)
...
""".strip().format(class_name=self.__class__.__name__,
    ...
    def process_order(self, event, order):
        ...
        if 'available_volume' not in event:
            max_volume = self.volume_limit * event.volume
            event['available_volume'] = max_volume       

        # price impact accounts for the total volume of transactions
        # created against the current minute bar
        event.available_volume = event.available_volume - self.volume_for_bar
        if event.available_volume <= 100:
            # we can't fill any more transactions
            return
        ...
        price = event.price + simulated_impact
        volume_affordable = math.floor(self.available_cash_for_bar/price/100.0)*100
            
        if order.direction > 0 and volume_affordable < 100:
            return

        if order.direction > 0:
            cur_volume = min(volume_affordable, cur_volume)
        return create_transaction(
            event,
            order,
            # In the future, we may want to change the next line
...
class FreeTradeSlippage(SlippageModel):

    def __init__(self, volume_limit=0.0, price_impact=0.0):
...
""".strip().format(class_name=self.__class__.__name__,
    ...
    def process_order(self, event, order):
        ...
        # the current order amount will be the min of the
        # volume available in the bar or the open amount.
        cur_volume = int(abs(order.open_amount))
