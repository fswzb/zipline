...
def search_day_distance(env, end_date, dt):
...
def choose_treasury(select_treasury, treasury_curves, start_date, end_date, env,
                    compound=True):
    ...
    if not search_day:
        ...
        for prev_day in search_days[i - 1::-1]:
            ...
            if rate is not None:
                search_day = prev_day
                search_dist = search_day_distance(env, end_date, prev_day)
                break
