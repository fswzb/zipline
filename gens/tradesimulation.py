...
class AlgorithmSimulator(object):
    ...
    def process_event(self, event):
        ...
        cash = self.algo.perf_tracker.todays_performance.starting_cash + self.algo.perf_tracker.todays_performance.period_cash_flow
        for txn, order in process_trade(event, cash):
    ...
    def transform(self, stream_in):
        ...
        with self.processor.threadbound():
            ...
            for date, snapshot in stream_in:
                ...
                #max_bar_count = self.algo.warming_period
                #if date < self.algo_start - datetime.timedelta(max_bar_count):
                    #print "pass: %s"%date
                    #continue  
                ...
                if date < self.algo_start:
                    ...
                    if self.algo.history_container:
                        self.algo.history_container.update(self.sim_params.sids, self.current_data, self.simulation_dt)                        
                else:
                    ...
                    if self.sim_params.data_frequency == 'minute':
                        ...
                        if date == mkt_close:
                            ...
                            if mkt_close <= self.algo.perf_tracker.last_close:
                                before_last_close = mkt_close < self.algo.perf_tracker.last_close
                                try:
                                    mkt_open, mkt_close = \
                                    self.algo.trading_environment.next_open_and_close(mkt_close)
                                    #trading.environment.next_open_and_close(mkt_close)
                                    print "new_mkt_open, mkt_close"           
                                ...
                                self.algo.perf_tracker.handle_intraday_market_close(mkt_open, mkt_close)
                                             
                    elif data_frequency == 'daily':
                        if date <= self.algo.perf_tracker.last_close:
                            before_last_close = date < self.algo.perf_tracker.last_close
                            try:
                                next_day = self.env.next_trading_day(date)
                            except trading.NoFurtherDataError:
                                pass

                            if next_day is not None and next_day <= self.algo.perf_tracker.last_close:
    ...
    def _process_snapshot(self, dt, snapshot, instant_fill):
        ...
        events_to_be_processed = list()
        trades_to_be_reprocessed = list()
        self.sim_params.active_sids = set()
        for event in snapshot:           
            if event.type == DATASOURCE_TYPE.TRADE:  
                #update price info for that day
                self.update_universe(event)
                any_trade_occurred = True
                trades_to_be_reprocessed.append(event)
        ...
        #process twice to make sure buy orders are filled as much as possible, will not execute order twice due to given order amount
        for trade in trades_to_be_reprocessed:
            self.process_event(trade)
                
        #market close, evening activity: place new orders for tomorrow               
        if any_trade_occurred:
            self.current_data = BarData({sid:sid_data for sid, sid_data \
            in self.current_data.iteritems() if 'price' in self.current_data[sid]})
