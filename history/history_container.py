...
class HistoryContainer(object):
    ...
    def update(self, current_sids, data, algo_dt):
        """
        Takes the bar at @algo_dt's @data, checks to see if we need to roll any
        new digests, then adds new data to the buffer panel.
        """
        to_add = current_sids.difference(self.sids)
        to_drop = self.sids.difference(current_sids)
        self.add_sids(to_add)
        self.drop_sids(to_drop)
