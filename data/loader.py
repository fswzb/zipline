...
def load_treasury(filename):
    
    #tr = pd.read_csv(filename)
    tr = pd.read_pickle(filename)
    #tr.Date = tr.Date.apply(lambda x:datetime.datetime.strptime(x,"%Y/%m/%d"))
    #tr.Date = tr.Date.apply(lambda x:pytz.utc.localize(x))
    #tr.index = tr.Date
    #tr['tid'] = range(1, len(tr) + 1)
    return tr


def load_benchmark(filename):
    """
    assert freq in ('daily', 'minute')  
    if freq == "minute":
        bm = pd.read_csv(filename, parse_dates=['Date'], index_col = 'Date')
        bm.index = map(lambda x:pytz.utc.localize(x), bm.index)
    else:
        bm = pd.read_csv(filename)
        bm.Date = bm.Date.apply(lambda x:datetime.datetime.strptime(x,"%Y/%m/%d"))
        bm.Date = bm.Date.apply(lambda x:pytz.utc.localize(x))
        bm.index = bm.Date
