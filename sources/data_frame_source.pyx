...
class DataFrameSource(DataSource):
    ...
    def raw_data_gen(self):
        cdef unsigned int rowidx, field_idx
        cdef str field
        for rowidx in range(len(self.arraydata)):
            series = self.arraydata[rowidx]
            event = {}
...
class DFSingleSource(DataSource):
    def __init__(self, data, **kwargs):
...
class DFSingleSource2(DataSource):
    def __init__(self, data, **kwargs):
        assert isinstance(data.index, pd.tseries.index.DatetimeIndex)
        self.data = data
        self.arraydata = data.values
